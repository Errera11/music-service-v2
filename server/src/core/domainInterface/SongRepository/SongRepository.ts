...
export interface SongRepository {
    ...
    getAll(skip: number, take: number): Promise<(Song & {isLiked: boolean})[]>
    getTrackById(id: number): Promise<Song>
    getUserSongs(userId: string, skip: number, take: number): Promise<(Song & {isLiked: boolean})[]>
    addToFavorite(userId: string, songId: number): Promise<any>
    searchSong(query: string, skip: number, take: number, userId?: string): Promise<(Song & {isLiked: boolean})[]>
}